 ...
 * @return {Integer}
 */
AlexaPropertyMap.prototype.getDeferredResponseTime = function(interfaceName) {
  var properties = this[interfaceName] || {};
  var parameter = 'deferredResponse';
  var maxDelay = interfaceName === 'LockController' ? 30 : 8;

  return Object.keys(properties).reduce(function(deferredResponseTime, propertyName) {
    var delay = Math.min(properties[propertyName].parameters[parameter] || 0, maxDelay);
    deferredResponseTime = Math.max(delay, deferredResponseTime);
    return deferredResponseTime;
  }, 0);
}
