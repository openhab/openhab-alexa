...
async function getSkillValidations(id) {
  ...
  // Retrieve skill validations up to 30 times every 30 seconds while status is in progress
  for (let tries = 0; (!validations || validations.status === 'IN_PROGRESS') && tries < 30; tries++) {
    // Wait for 30 seconds
    await sleep(30000);
