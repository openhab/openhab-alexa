...
The below example exposes 3 seperate single endpoints, with "Light 1" and "Light 2" as independent lights with brightness and on/off controls, while and "Light Group" controlling these two as a group of lights with on/off control.
...
String Navigate "Navigate" (Stereo)  {alexa="Navigation" [supportedKeys="UP,DOWN,LEFT,RIGHT,SELECT"]}
...
String Mode     "Mode"     (Stereo)  {alexa="EqualizerMode" [supportedModes="MOVIE,MUSIC,TV"]}
```
...
String ArmMode             "Arm Mode"        (SecurityPanel) {alexa="ArmState" [supportedArmStates="DISARMED,ARMED_STAY,ARMED_AWAY"]}
...
```
Group  Washer      "Washer"               {alexa="Other"}
...
A device type represents an [Alexa display category](https://developer.amazon.com/docs/device-apis/alexa-discovery.html#display-categories) that affects how the endpoint appears in the Alexa app and in some cases, the associated features.
...
`Computer` | _[`PowerState`](#powerstate)_ | A non-mobile computer, such as a desktop computer.
...
`GameConsole` | _[`PowerState`](#powerstate)_ | A game console, such as Microsoft Xbox or Nintendo Switch.
...
`Headphones` | _[`PowerState`](#powerstate)_, _[`VolumeLevel`](#volumelevel)_, [`MuteState`](#mutestate), [`Playback`](#playback), [`PlaybackStop`](#playbackstop), [`BatteryLevel`](#batterylevel) | A wearable device that transmits audio directly into the ear.
`Hub` | _[`PowerState`](#powerstate)_ | A smart-home hub.
`Laptop` | _[`PowerState`](#powerstate)_, [`BatteryLevel`](#batterylevel) | A laptop or other mobile computer.
...
`MobilePhone` | _[`PowerState`](#powerstate)_, [`BatteryLevel`](#batterylevel) | A mobile phone.
...
`Router` | _[`PowerState`](#powerstate)_ | A network router.
...
`Tablet` | _[`PowerState`](#powerstate)_,  [`BatteryLevel`](#batterylevel) | A tablet computer.
...
`Wearable`| _[`PowerState`](#powerstate)_, [`BatteryLevel`](#batterylevel) | A network-connected wearable device, such as an Apple Watch, Fitbit, or Samsung Gear.
...
For device attributes used in a group endpoint, if multiple items with the same attribute are part of a group, only the first one will be considered while the others will be ignored. This doesn't apply to [generic attributes](#genericattributes).
...
The setup is a bit complex as it requires to use the [IP Camera binding](https://www.openhab.org/addons/bindings/ipcamera/) to create a [HLS live stream](https://www.openhab.org/addons/bindings/ipcamera/#hls-http-live-streaming) that can be accessed over HTTPS on your local network on port 443 using a valid SSL certificate. It is highly recommended using a reverse proxy, such as NGINX, for this setup and [Let's Encrypt](https://letsencrypt.org) to validate your certificate.

Once setup, configure the item linked to the binding `hlsUrl` channel and set the metadata parameter `proxyBaseUrl="https://cam1.myserver.com"`. This will cause the skill to provide a stream URL to your Echo device, when requested, that should be accessible at `https://cam1.myserver.com/ipcamera.m3u8` along with a static image URL, replacing the extension to `.jpg`. If your server requires basic authentication, set the `username` and `password` parameters.
...
#### `ObstacleAlert`
Items that represent the alert state when an obstacle is detected preventing closing a garage door. This needs to be paired with [`OpenState`](#openstate). This feature only works with [`GarageDoor`](#device-types) type. When in alert state, close requests will trigger an error response.
...
#### `SafetyBeamAlert`
Items that represent the alert state when a safety beam was breached preventing closing a garage door. This needs to be paired with [`OpenState`](#openstate). This feature only works with [`GarageDoor`](#device-types) type. When in alert state, close requests will trigger an error response.
...
If paired with [`TiltAngle`](#tiltangle), the primary controls (open/close/stop) can be defined as position-based or tilt-based setting metadata parameters `primaryControl="position"` at the group level. It is important to note that raise/lower controls aren't affected by these settings and will remain position-based.
...
* Supported metadata parameters:
  * inverted=`<boolean>`
    * set to true to invert state
    * defaults to true (Rollershutter); false (Dimmer)
  * primaryControl=`<type>`
  ...
  * presets=`<presets>`
    ...
    * pre-defined [asset ids](#asset-catalog), prefixed with an @ sign
...
If paired with [`PositionState`](#positionstate), the primary controls (open/close/stop) can be defined as position-based or tilt-based setting metadata parameters `primaryControl="tilt"` at the group level.
...
* Supported metadata parameters:
  * inverted=`<boolean>`
    * set to true to invert state
    * defaults to true (Rollershutter); false (Number*/Switch)
  * primaryControl=`<type>`
    * supported types are `position` or `tilt`
    * defaults to `position`
  * presets=`<presets>`
...
* Utterance examples:
  * *Alexa, change the input to DVD on the `<device name>`.*
  * *Alexa, change the channel to 200 on the `<device name>`.*
  * *Alexa, change the channel to PBS on the `<device name>`.*
...
#### `Input`
Items that represent a source input (e.g. "HDMI 1", or "TUNER" on a stereo).

* Supported item types:
  * String
* Supported metadata parameters:
  * supportedInputs=`<inputs>`
    * comma delimited list (e.g. `supportedInputs="HMDI1,TV,XBOX"`)
    * [supported input values](https://developer.amazon.com/docs/device-apis/alexa-inputcontroller.html#input-values)
    * space between the input name and number is not sent to openHAB (e.g. "HDMI 1" [Alexa] => "HDMI1" [OH])
    * required list of supported input values
* Utterance examples:
  * *Alexa, change the input to DVD on the `<device name>`.*
...
* Supported metadata parameters:
  * range=`<minValue:maxValue>`
    ...
    * defaults to `"0:100"` (Dimmer); `"-10:10"` (Number)
  * defaultLevel=`<number>`
    * defaults to midpoint range spread
  * increment=`<number>`
    * defaults to INCREASE/DECREASE (Dimmer); 1 (Number)
* Utterance examples:
...
* Supported item types:
  * String [UP="up", DOWN="down", LEFT="left", RIGHT="right", SELECT="select", PAGE_UP="pageup", PAGE_DOWN="pagedown", PAGE_LEFT="pageleft", PAGE_RIGHT="pageright", INFO="info", MORE="more"]
* Supported metadata parameters:
  * UP=`<command>`
  * DOWN=`<command>`
  * LEFT=`<command>`
  ...
  * supportedKeys=`<keys>`
    ...
    * supported keys are UP, DOWN, LEFT, RIGHT, SELECT, PAGE_UP, PAGE_DOWN, PAGE_LEFT, PAGE_RIGHT, INFO, MORE
    * defaults to, depending on the parameters provided, either user-based or item type-based default mappings.
* Utterance examples:
  * *Alexa, scroll right.*
  * *Alexa, page down.*
...
* Supported metadata parameters:
  * increment=`<number>`
    ...
    * defaults to INCREASE/DECREASE (Dimmer); 500 (Number)
  * range=`<range>`
    ...
    * defaults to, depending on the parameters provided in order of precedence:
      * preset-based using the binding linked name and thing type (via channel metadata), to differentiate color/white ranges, defaulting to color.
      * default to `1000:10000`
* Utterance examples:
...
* Utterance examples:
  ...
  * *Alexa, what's the `<device_name>` set to?*
...
* Supported metadata parameters:
  ...
  * supportedModes=`<values>`
    ...
    * defaults to, depending on the parameters provided in order of precedence:
      ...
      * preset-based related to the thermostat binding linked (via channel metadata)
...
* Supported metadata parameters:
  ...
  * unitOfMeasure=`<unitId>`
    ...
    * defaults to unit of item state presentation, if available, or item dimension unit of measurement as follow:
...
InputController | [`Input`](#input) | `de-DE`, `en-AU`, `en-CA`, `en-GB`, `en-IN`, `en-US`
...
ModeController | [`Mode`](#mode), [`FanDirection`](#fandirection), [`FanSpeed`](#fanspeed), [`OpenState`](#openstate), [`PositionState`](#positionstate), [`TiltAngle`](#tiltangle), [`ThermostatFan`](#thermostatfan), [`VacuumMode`](#vacuummode) | `de-DE`, `en-AU`, `en-CA`, `en-GB`, `en-IN`, `en-US`, `es-ES`, `es-MX`, `fr-CA`, `fr-FR`, `it-IT`, `ja-JP`
