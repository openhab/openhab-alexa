...
class ColorTemperature extends AlexaProperty {
  ...
  /**
   * Returns default range based on binding name and thing type to differentiate white & color ranges
   * @return {Array}
   */
  get defaultRange() {
    const { binding, thingType } = this.item;
    switch (binding) {
      case Binding.HUE:
        return ['0220'].includes(thingType) ? [2200, 6500] : [2000, 6500];
      case Binding.LIFX:
        return ['whitelight'].includes(thingType) ? [2700, 6500] : [2500, 9000];
      case Binding.MILIGHT:
        return ['whiteLed'].includes(thingType) ? [2700, 6500] : [2700, 6500];
      case Binding.TPLINK:
        return ['kl120', 'lb130'].includes(thingType) ? [2700, 6500] : [2500, 9000];
      case Binding.TRADFRI:
        return ['0220'].includes(thingType) ? [2200, 4000] : [1780, 6000];
      case Binding.YEELIGHT:
        return ['ceiling', 'dolphin'].includes(thingType) ? [2700, 6500] : [1700, 6500];
      default:
  ...
  get hasSaturationColorMode() {
    return this.item.binding !== Binding.HUE;
  }
