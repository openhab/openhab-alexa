...
class Discovery extends AlexaHandler {
  ...
  static async discover(directive, openhab) {
    ...
    // Throw type error if items retrieved not an array
    ...
    for (const item of items) {
      ...
      // Skip item if endpoint not defined or if already part of a group
      if (!endpoint || groupItems.includes(item.name)) {
        continue;
      }

      // Add group children capabilities if endpoint group
      if (endpoint.isGroup) {
        items
          .filter((member) => member.groupNames && member.groupNames.includes(item.name))
          .forEach((member) => {
