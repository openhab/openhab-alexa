...
class ThermostatController extends AlexaHandler {
  ...
  static async setTargetTemperature(directive, openhab) {
    ...
    // Get current alexa thermostat mode if property defined and supports setpoint mode
    const mode = await (thermostatMode &&
      thermostatMode.supportsSetpointMode &&
      openhab.getItemState(thermostatMode.item.name).then((state) => thermostatMode.getState(state)));
  ...
  static async adjustTargetTemperature(directive, openhab) {
    ...
    // Get current alexa thermostat mode if property defined and supports setpoint mode
    const mode = await (thermostatMode && thermostatMode.supportsSetpointMode !== false
      ? openhab.getItemState(thermostatMode.item.name).then((state) => thermostatMode.getState(state))
      : Promise.resolve());
  ...
  static async resumeSchedule(directive, openhab) {
    ...
    // Get current alexa thermostat mode
    const mode = await openhab.getItemState(thermostatMode.item.name).then((state) => thermostatMode.getState(state));
