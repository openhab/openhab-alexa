...
/**
 * Defines catalog schema path
 * @type {String}
 */
const CATALOG_SCHEMA_PATH = path.join('..', 'lambda', 'catalog.json');

/**
 * Defines skill manifest schema path
 * @type {String}
 */
...
function updateCatalogSchema() {
  ...
  for (const locale of SUPPORTED_SKILL_LOCALES) {
    ...
    if (fs.existsSync(catalogPath)) {
      ...
      for (const [assetId, values] of Object.entries(assetIds)) {
        const labels = values.map((text) => ({ text, locale }));
        schema[assetId] = [...(schema[assetId] || []), ...labels];
      }
    }
  }
  // Save catalog schema
  saveSchema(schema, CATALOG_SCHEMA_PATH);
}

/**
 * Update skill manifest schema
 */
function updateSkillManifestSchema() {
